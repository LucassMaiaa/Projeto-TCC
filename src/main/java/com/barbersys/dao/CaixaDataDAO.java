...
public class CaixaDataDAO {
	...
	// Verifica se existe registro de caixa para a data selecionada
	...
	// Atualiza valores e status do caixa
	...
	// Salva novo registro de caixa
	public static void salvar(CaixaData caixa) {
		String sql = "INSERT INTO caixadata (cai_valor_inicial, cai_valor_final, cai_data, cai_status) VALUES (?, ?, ?, ?)";
		try (Connection conn = DatabaseConnection.getConnection();
			 PreparedStatement stmt = conn.prepareStatement(sql)) {
